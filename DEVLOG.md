...
| 세션 시작 (채널 포함) | 대시보드 요청 → 창 기동 → 채널 등록 → 즉시 전송까지 22.1초 |
...
- **셸 heredoc 안에서 백슬래시가 반복적으로 뭉개졌습니다.** `'*\\WindowsApps\\...'` 로
  ...
  파일에서 읽어오는 편이 안전합니다.
- **문자열 안 줄바꿈 이스케이프를 네 번 뭉갰습니다.** 셸 heredoc 을 거치며 `\n` 이
  실제 줄바꿈이 되어 문자열 리터럴이 깨졌습니다. 마지막에는 `web/app.js` 전체가
  파싱 실패해 대시보드가 통째로 빈 화면이 됐습니다. 서버는 멀쩡했는데 화면만 죽어서
  원인을 찾는 데 시간이 걸렸습니다.
  **대책:** 소스에 이스케이프를 쓰지 않는다. `String.fromCharCode(10)` 같은 걸로
  조립하거나, 파일을 통째로 교체한다. 그리고 UI 를 고칠 때마다 `node --check` 를 돌린다.
- **함수를 통째로 날렸습니다.** 레인 렌더링을 앱/탭으로 가르면서 파이썬으로 코드 블록을
  구간 치환했는데, 그 구간 안에 있던 `renderAdapters` 와 상수들이 같이 지워졌습니다.
  호출부만 남아 연결 섹션이 조용히 비어 있었고, 문법 오류가 아니라서 바로 안 드러났습니다.
  **대책:** 구간 치환 대신 앵커 한 줄을 잡아 삽입/교체한다. 큰 블록을 바꿀 땐
  바꾸기 전후로 남아 있어야 할 심볼을 `grep -c` 로 확인한다.
...
### 6.4 세션 시작 기능 — 창 띄우기에서 두 번 걸렸다

대시보드에서 채널이 붙은 세션을 띄우는 기능(`server/sessionLauncher.js`).
핵심은 환경변수 정리인데, 정작 시간을 잡아먹은 건 **창을 띄우는 것** 자체였다.

**1) start 의 인자로 명령을 넘기면 깨진다**

```js
spawn('cmd.exe', ['/c', 'start', title, 'cmd', '/k', `cd /d "${cwd}" && claude ...`])
```

공백과 `&&` 가 그대로 살아남지 못해 창이 아예 안 떴다.
→ 명령을 임시 배치 파일로 빼서 해결.

**2) 숨김 창으로 실행 중인 서버의 자식 콘솔은 화면에 안 나온다**

`cmd /c start` 로 띄우면 콘솔이 있는 곳에서는 창이 뜨는데, FleetView 서버(숨김 창으로
실행 중)에서 호출하면 프로세스만 살아있고 창이 보이지 않았다. 자식이 서버의 창 상태를
물려받는다.

```js
// 서버가 숨김 창이면 창이 안 보인다
spawn('cmd.exe', ['/c', 'start', '""', script])
// 탐색기는 사용자의 대화형 세션에서 새로 실행해 주므로 서버 상태와 무관하다
spawn('explorer.exe', [script])
```

**교훈:** 창을 띄우는 코드는 **그 코드가 어디서 실행되는지**에 따라 결과가 달라진다.
콘솔에서 손으로 시험해 성공한 것이 서비스에서도 되리라 가정하면 안 된다.
...
FleetView 서버가 다른 Claude Code 세션 안에서 시작됐다면 이 표시들이 환경에 남아 있고,
그대로 물려주면 새 세션이 자식 세션으로 인식되어 트랜스크립트가 저장되지 않는다.
그러면 `~/.claude/sessions/<pid>.json` 도 안 생겨 채널이 자기 세션을 못 찾는다.

**검증 완료:** 폴더를 지정해 요청하면 22.1초 만에 창이 뜨고 채널이 붙어
세션 id 를 돌려준다. 이어서 그 세션에 보낸 메시지에 `"시작 확인됨"` 이 회수됐다.

### 6.5 세션 바로가기 — 앱은 창이 하나뿐이다

세션 카드의 `→` 는 원래 무조건 새 터미널을 `--resume` 으로 열었다. 살아 있는 세션에
그러면 프로세스가 하나 더 생겨 기록이 갈라진다. 상태에 따라 갈라 처리하도록 고쳤다.

| 세션 상태 | 동작 |
|---|---|
| 살아 있음 + 창 찾음 | 그 창을 앞으로 (`SetForegroundWindow`) |
| 살아 있음 + 창 못 찾음 | 안내만 한다. **새 창을 띄우지 않는다** |
| 꺼져 있음 | 터미널에서 `--resume` 으로 이어 연다 |

창 찾는 방법은 세션 종류마다 다르다.

- **Claude 데스크톱 앱 세션**: 세션의 `claude.exe` 는 창이 없고 부모인 앱이 창을 가진다.
  프로세스 조상을 거슬러 올라가 찾는다. 동작 확인됨.
- **터미널 세션**: 콘솔 호스트(Windows Terminal)가 별도 프로세스라 조상 추적으로
  안 잡힌다. 창 제목으로도 못 찾았다 — Claude Code 가 제목을 자기 것으로 덮어쓴다.
  이 경우 안내만 하고 사용자가 직접 전환하게 둔다.

**한계:** Claude 앱은 보이는 최상위 창이 **하나뿐**이고 세션들은 그 안에서 탭으로 갈린다.
그래서 바로가기는 앱 창까지만 띄우고, 세션 탭은 사용자가 직접 골라야 한다.

```
EnumWindows 결과: hwnd=1180320 pid=22104 title=[Claude]   ← 창이 이것 하나
```

### 6.6 답장 도구에만 의존하면 답이 안 온다

채널로 메시지를 넣으면 Claude 가 `fleetview_reply` 도구를 불러 답을 돌려주도록 설계했다.
서버 지시문에 "반드시 호출하라" 고 적었지만 **강제할 수단이 없다.**

실제로 짧은 대화에서는 그냥 화면에만 답하고 도구를 부르지 않았다.

```
15:23:18  <channel ...> 아
15:23:21  "특별히 도와드릴 작업이 있으신가요?"   ← 3초 만에 답했지만 도구는 안 부름
```

Claude 는 3초 만에 답했는데 대시보드는 계속 기다렸다. 사용자에게는
"채널이 더 느리다" 로 보였다. 느린 게 아니라 **답이 영영 안 오는** 것이었다.

**수정:** 답을 받는 길을 둘로 만들었다.

1. 도구를 부르면 그걸 쓴다 (정확)
2. 안 부르면 세션 기록 파일에서 새로 생긴 응답을 주워 온다
   (1.5초마다 확인, 3초간 변하지 않으면 완성으로 본다)

기록 파일은 세션 목록을 만들 때 어차피 읽고 있으므로 추가 비용이 없다.

**교훈:** 모델에게 "반드시 이 도구를 부르라" 고 시키는 설계는 그것만으로 완결되면 안 된다.
안 불렀을 때의 길을 반드시 함께 만들어야 한다.
