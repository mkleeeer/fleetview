...
**그리고 그 길도 정확해야 한다.** 처음에는 "기록의 마지막 어시스턴트 발화" 를 주웠는데,
사용자가 그 창에서 동시에 다른 걸 물어보면 그 답을 가로채고, 도구를 여러 번 쓰는 도중의
중간 말을 최종 답으로 착각한다. 사용자가 이 위험을 지적해서 고쳤다.

채널 메시지는 기록에 `<channel ... msg_id="msg_xxx">` 로 남는다. 그 항목을 찾아
**그 뒤부터 다음 사용자 발화 전까지**의 어시스턴트 발화만 모은다. 다음 사용자 발화가
나타나면 턴이 끝난 것이므로 즉시 확정하고, 아직이면 잠시 변화가 없을 때까지 기다린다.

```
msg_gassey5c | 턴끝남 true  | "아직 구체적인 요청 내용이 없어 대기하고 있습니다..."
msg_bv3vxhnq | 턴끝남 false | "확인"
```

### 6.7 느린 게 아니라 내가 기다린 것이었다

사용자가 "대시보드가 7초쯤 느리다" 고 했다. 모델이 느린 게 아니라
**답이 끝난 뒤 내가 더 기다리고 있었다.**

완료 판정을 "1.5초마다 확인해서 3번 연속 변화가 없으면 끝" 으로 잡았기 때문이다.
답이 3초에 끝나도 확정까지 6초가 더 걸렸다.

기록을 보니 Claude Code 는 턴을 마치면 `system` 항목을 남긴다.

```
user       15:35:58  <channel ... msg_id="...">
assistant  15:36:01  "3"            <- 답 (3초)
attachment 15:36:03
system     15:36:03                 <- 턴 종료
```

이걸 보면 기다릴 필요가 없다. 확인 주기도 1.5초 -> 0.7초로 줄였다.
왕복 10초 -> 5초.

**함께 잡은 버그:** 채널 메시지를 대화 경계로 치지 않아서, 연속으로 보내면
이전 턴의 답까지 같이 딸려왔다(`"확인

3"`). `isRealUserText` 가
`<channel` 을 걸러내도록 만들어 둔 것이 여기서는 반대로 작용했다.
경계 판정에서는 채널 메시지도 경계로 본다.

**교훈:** "느리다" 는 신고를 받으면 상대 탓을 하기 전에 내 대기 로직부터 본다.
그리고 어떤 조건을 한 곳에서 걸러냈다면, 다른 곳에서 그 조건이 필요한지 확인한다.
