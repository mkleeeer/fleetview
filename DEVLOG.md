...
**2) start 에 /D 와 중첩 cmd /k 를 붙이면 창이 안 뜬다**

```js
// 안 됨
spawn('cmd.exe', ['/c', 'start', '""', '/D', cwd, 'cmd.exe', '/k', script])
// 됨
spawn('cmd.exe', ['/c', 'start', '""', script])
```

배치 파일 안에서 `cd /d` 를 하므로 `/D` 도 중첩 `cmd /k` 도 필요 없었다.
...
**미검증:** 창이 뜨는 것까지는 확인했으나, 시작 화면(브라우저 도구 확인 등)에서
사람이 한 번 눌러야 채널이 붙는다. 그 뒤 등록까지 이어지는 것은 아직 못 봤다.
타임아웃 시 그 상황을 안내하는 오류는 정상 동작한다.
